...
export default function AnimalForm({
  ...
  function validar() {
    ...
    if (!form.colorpelaje.trim())
      nuevosErrores.colorpelaje = "El color de pelaje es obligatorio.";
    if (form.peso === "" || Number(form.peso) <= 0)
      nuevosErrores.peso = "Ingresá un peso válido.";
    if (!form.lugarorigen.trim())
      nuevosErrores.lugarorigen = "El lugar de rescate es obligatorio.";
            ...
            <Select value={form.idespecie} onValueChange={set("idespecie")}>
              <SelectTrigger className="w-full">
                <SelectValue placeholder="Seleccioná una opción">
                  {
                    ESPECIES.find((e) => String(e.idespecie) === form.idespecie)
                      ?.nombre
                  }
                </SelectValue>
              </SelectTrigger>
            ...
            <Select value={form.estado} onValueChange={set("estado")}>
              <SelectTrigger className="w-full">
                <SelectValue placeholder="Selecciona una especia/raza">
                  {ESTADOS_ANIMAL.find((e) => e.value === form.estado)?.label}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
        ...
        <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Campo label="Veterinaria asignada (opcional)">
            <Select
              ...
              onValueChange={set("veterinariaId")}
            >
              <SelectTrigger className="w-full">
                <SelectValue placeholder="Sin asignar">
                  {form.veterinariaId &&
                    VETERINARIAS.find(
                      (v) => String(v.idresponsable) === form.veterinariaId,
                    )?.nombre}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
          ...
          <Campo label="Transitante / adoptante (opcional)">
            <Select
              ...
              onValueChange={set("responsableId")}
            >
              <SelectTrigger className="w-full">
                <SelectValue placeholder="Sin asignar">
                  {form.responsableId &&
                    RESPONSABLES_HOGAR.find(
                      (r) => String(r.idpersona) === form.responsableId,
                    )?.nombre}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
