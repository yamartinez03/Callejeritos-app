...
import { PawPrint, AlertTriangle, CheckCircle2, ArrowRight, Pencil, Trash2 } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Button } from "@/components/ui/button";
import EliminarAnimalDialog from "./EliminarAnimalDialog";
import { getBadgePrincipal, getBadgeSecundaria, formatMoneda, formatSexo, formatEdad } from "@/lib/animalUtils";
import { ESPECIES } from "@/shared/data/mockAnimales";

function nombreEspecie(idespecie) {
  return ESPECIES.find((e) => e.idespecie === idespecie)?.nombre ?? "Sin especificar";
}
  ...
  return (
    <Card className="flex h-full flex-col overflow-hidden py-0 transition-shadow hover:shadow-md">
      ...
      <div className="relative flex h-32 shrink-0 items-center justify-center bg-muted sm:h-36">
        <PawPrint className="h-12 w-12 text-muted-foreground/50 sm:h-14 sm:w-14" strokeWidth={1.5} />
        <span
      ...
      <CardContent className="flex flex-1 flex-col gap-3 p-4">
        <div>
          <h3 className="font-semibold text-card-foreground">{animal.nombre}</h3>
          <p className="text-sm text-muted-foreground">
            {nombreEspecie(animal.idespecie)} · {formatSexo(animal.sexo)} · {formatEdad(animal.edadestimada)} ·{" "}
            {animal.castrado ? "Castrado/a" : "Sin castrar"}
          </p>
        </div>

        {animal.responsable && (
          <p className="text-sm font-medium text-card-foreground">{animal.responsable.nombre}</p>
        )}
          ...
          <ul className="space-y-1">
            {animal.alertasMedicas.map((alerta, i) => (
              <li key={i} className="flex items-start gap-1.5 text-xs">
                ...
                <span className={alerta.vencida ? "text-destructive" : "text-muted-foreground"}>{alerta.label}</span>
              </li>
            ))}
          </ul>
        )}
        ...
        {badgeSecundaria && (
          <span className={`inline-block w-fit rounded-full px-2.5 py-0.5 text-[11px] font-medium ${badgeSecundaria.className}`}>
        ...
        <div className="mt-auto flex items-center justify-between gap-2 pt-1">
          <div className="min-w-0">
            <p className="text-[11px] uppercase tracking-wide text-muted-foreground">Costo total</p>
            <p className="truncate text-sm font-semibold text-destructive">{formatMoneda(animal.costoTotal)}</p>
          </div>
